...
var variableStates = ['string', 'variableName', 'variableEnd'];
var variableSymbols = ['var ', ';', '\n'];
...
var machine = {
  ...
  switchVariableState(state, symbol) {
    var stateIndex = variableStates.indexOf(state);
    var symbolIndex = variableSymbols.indexOf(symbol);
    switch (stateIndex) {
      case 0:
        if (symbolIndex === 0) {
          return methodStates[1];
      ...
      case 2:
        return methodStates[0];
      default:
        throw new Error('Unexpected variable state.');
    }
  }
}
...
class Formatter {
  ...
  formatVariable (str) {
    ...
    var state = 'string';
    for (var i = 0, len = str.length; i < len;) {
      var symbol = this.getSymbol(variableSymbols , str, i);
      state = machine.switchVariableState(state, symbol);
      switch (state) {
        case 'string':
          body += symbol;
          break;
