...
### 2015-06-24
解析器部分已经结束，接下来就要进行if节点行为的调整，以及design测试，发布即将到来。

### 2015-09-07
忽然发现这个项目也进行来那么久了，加上最近工作也挺着急的，感觉真难玩儿啊。
这边进行et的设计改版已经进行了将近两周了，但是还是没有做好。

打算的是在create和update遍历里面再加入append和remove的遍历，
主要是解决一些条件判断造成的节点更改。

append函数主要会管理两个方面的事情，一个是与父节点创建树形关系，第二个是管理root节点。
root节点的主要作用在于get和remove函数能够正确快速得把当前模板在页面中的节点移除掉。

remove函数管理一个事情，那就是执行移除当前节点，按道理只有root节点才会调用到这个函数，
因为其他节点都是跟着父亲节点被管理的。

For模板想来想去还是完全按照一个新的模板来创建，包括模板的root行为也是的。

其实这次改版主要进行的任务是把if和for两个遍历重新设计，
首先是if不使用新模板的方式，而是进行代码解释的方式，这样不会产生多余的模板，能压缩代码提高性能；
for节点也不使用新的模板对象，但是其下面的内容独立称为一个新的模板对象。

### 2015-10-11 关于for节点的处理
For节点的处理有个问题存在，轮询出来的所有节点都使用新的模板，而for节点本身也需要使用新模板吗？

如果不，那么for节点和if节点的处理方式就会不一样，会使用before去插入新的节点，因为before类似于append不会产生节点的乱序，而after函数其实类似于prepend。但是我不喜欢这样，因为这样就产生了多余的差异，我希望整合这样的差异。

那么，如果使用新模板来处理，那么又存在另外一个问题，那就是节点需要整体处理，需要整个节点重新从模板拼装出来，然后放到标记线上去。这样在界面上来说是非常不合理的一件事情，因为前面没有变化的数据节点也受到影响了。

想了一下，想要所有节点都使用before来处理就好了，但是每个节点都是有顺序的，每个ID的大小代表了节点的插入顺序。所以对于if节点而言，必须使用after函数来处理内容模板。

最后的解决办法，在每个模板添加一个模板的结束线，这样就能新增一个appendAfterTemplate的方法，插入到模板后面去。Yes就是这样，然后for轮询也能够使用after方法进行插入了。
